	...
	if (validURL(query)) {
		try {
			...
			if (playerConfig.volume === "auto") {
				...
				try {
					const userConfig = DataBase && DataBase.ZiUser 
						? await DataBase.ZiUser.findOne({ userID: user.id }) 
						: null;
				
					playerConfig.volume = userConfig?.volume ?? DefaultPlayerConfig.volume;
				} catch (error) {
					console.error('Error fetching user configuration:', error);
					playerConfig.volume = DefaultPlayerConfig.volume;
				}
			}
